...
####################################################
    ...
    print(LA.norm(error))
    if LA.norm(error)!=0:
        print('model substitude successfully')
...
##########################################
        if model.fc1.weight.requires_grad == True and Update_last_core == True:
            ...
            Q = rreshape(Loss_grad,[np.prod(reshape_size[:-1]),reshape_size[-1]])
            #[n1n2n3,n4]
            Q = torch.Tensor(Q)
            Size_temp = np.append((A1.t().mm(Q)).shape,1)
            #[r4,n4,r5=1]
            G[-1] = G[-1]+ rreshape(A1.t().mm(Q),Size_temp)
            #print(LA.norm(A1.t().mm(Q)),'bp added norm')
            W = rreshape(torch.Tensor(TT.ProTTSVD(G)),model.fc1.weight.data.shape)
...
# Initialize the grad
###########################################
            model.fc1.weight.grad = torch.zeros(model.fc1.weight.grad.shape)
            model.fc1.bias.grad = torch.zeros(model.fc1.bias.grad.shape)
            model = model.to('cuda')
